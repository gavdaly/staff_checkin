...
) -> Result<crate::models::ApiPeriodV2010PeriodAccountPeriodKey, Error<FetchKeyError>> {
    let local_var_configuration = configuration;
...
) -> Result<crate::models::ListKeyResponse, Error<ListKeyError>> {
    let local_var_configuration = configuration;
...
pub async fn update_key(
...
) -> Result<crate::models::ApiPeriodV2010PeriodAccountPeriodKey, Error<UpdateKeyError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<UpdateKeyError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
